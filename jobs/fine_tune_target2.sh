...
for i in ${!seed[@]}; do
    for j in ${!train_state[@]}; do
        for k in ${!n_samples[@]}; do
            exp_dir=$resnet'_source-IL_target-'${train_state[$j]}'_ft-block2_ns-all_nt-'${n_samples[$k]}
        ...
        exp_dir=$resnet'_source-IL_target-'${train_state[$j]}'_ft-block2_ns-all_nt-all'
        model_fname=$model_path'/source_checkpoint_'${seed[$i]}'.pt'
        echo $exp_dir
